...
* C 编译器负责把C代码翻译成特定的机器语言。此外，C编译器还将源代码与C库（库中包含大量的标准函数供用户使用，如`printf()`和`scanf()`）的代码合并成最终的程序（更精确地说，应该是由一个被称为链接器的程序来链接库函数，但是在大多数系统中，编译器运行链接器）。
...
![image-20201228214316027](./image/image-20201228214316027.png)
...
* ==声明==是C语言最重要的特性之一，编译器通过变量的类型信息在内存中分配存储空间。**这一片空间在用户方的名字就是变量名，在机器中则是对应的地址。**声明把特定标识符与计算机内存中的特定位置联系起来，同时也确定了储存在某位置的信息类型或数据类型。
...
* `scanf`函数的几点说明
  * `scanf(" ... ", ... , ... ,....)`
  * `scanf`接受的数据类型是**字符串和指针**
  * 接受多少个指针类型与字符串中有多少个占位符有关
  * `scanf`只接受输入，无法输出
  * 如果格式符中没有间隔，可以在输入时用空格或回车作为输入的间隔
* `float`类型可以存储带小数的数字，在`printf`中使用 `%f`来打印`float`类型的变量，且可以使用`%.nf`控制精确输出小数点后n位的数据
  * 不要在浮点型常量中间加空格
...
* `int`：有符号整型。存储一个`int`要占用一个机器字长
  * 声明`int`变量：可以一次一个，也可以一次多个
  * 变量获取值有3种方法
    * 赋值
    * 通过函数(如 `scanf`) 获得值
    * 初始化变量
...
* `short int`（或者简写为`short`）常用于较小数值的场合以节省空间。有符号
...
* 整数溢出时会重新从起始点开始。`unsigned int` 从0开始，`int`从负数开始

* 打印 `unsigned int`，使用`%u`；打印long，使用`%ld( %lo, %lx)`

* 打印`short`，使用 `%hd`(` %do`,`%hx`)

* `char` 类型用于储存字符，==是整数类型==，实际存储的是整数而不是字符。计算机使用数字编码来处理字符。在`printf`中使用 `%c`打印
...
* 看下面的例子
  ...
  const double* pd = rates; // pd指向数组的首元素地址，此时pd=&rates[0]=rates。const意味着不可以通过指针pd来修改rates中的内容，但是可以修改pd本身
...
* `const`还可以初始化一个不能指向别处的指针，**关键是const的位置**
...
2. 字符串数组和初始化
   ...
   * 定义字符串数组时，必须让编译器知道需要多少空间
     * 通常让编译器确定数组的大小很方便，省略数组初始化声明中的大小
...
3. 数组和指针
   ...
   * 数组形式和指针形式有何不同？
     * **在数组形式中，`ar1`是地址常量**，不能更改`ar1`。在这种情况下，字符串有两个副本。一个是在静态内存中的字符串字面量，另一个是储存在`ar1`数组中的字符串。
   ...
   * 总之，**初始化数组把静态存储区的字符串拷贝到数组中，数组名是一个新的地址常量（相当于双引号括起的内容）；而初始化指针只把静态存储区的字符串地址拷贝给指针变量，其地址仍为静态存储区的地址。**
...
4. 数组和指针的区别
   ...
   * 两者的主要区别是：数组名`heart`是常量，相当于对"I love Tille!"（即首元素地址）起了个别名；而指针名`head`是变量，是一个指向"I love Tille!"首元素的指针

5. 字符串数组

   * 创建字符串数组的方法：
     * 指向字符串的指针数组，数组的每一个元素都是一个指针
     * `char`类型的数组的数组，数组的每一个元素都是一个数组
...
![image-20210225211708630](note%20for%20C.assets/image-20210225211708630.png)

* mesg和copy是两个不同的指针，存储在不同的位置（&mesg ~= &copy），但他们的值相同（mesg == copy），也就是说它们指向了相同的位置。因此，程序并未拷贝字符串，语句`copy = mesg;`把mesg的值赋给copy，让copy也指向mesg指向的字符串。
...
* 在输入行不溢出的情况下，`fgets()`函数把换行符放在字符串的末尾，通常要与`fputs()`函数配对使用。

* `fputs()`的第2个参数指明它要写入的文件。如果要显示在计算机显示器上，应使用`stdout`（标准输出）作为该参数。`fputs()`与`puts()`的区别在于==`fputs()`不会在输出时添加换行符，`puts()`则会在待输出字符串末尾自动添加一个换行符。==
...
* 如果目标存储区装得下输入行，3个函数都没有问题，**但是`fgets()`会保留输入末尾的换行符作为字符串的一部分，**要编写额外的代码将其替换成空字符。
* 当
...
* `strcmp()`函数是根据ACSII码的值来比较两个字符串的；`strcmp()`函数首先将s1字符串的第一个字符值减去s2第一个字符，**若差值为零则继续比较下去；若差值不为零，则返回差值。**只要出现了不同的字符，比较就结束了
...
* 大多数情况下，`strcmp()`返回的具体值并不重要，我们只在意该值是0还是非0，即比较的两个字符串是否相等
...
* 而`strncmp()`函数在比较两个字符串时，可以比较到字符不同的地方，也可以只比较第3个参数指定的字符数。
  * `strncmp(A,B,5)`即比较A和B的前5个元素（不是第5个）
...
* `strcpy()`接受两个字符串指针作为参数，可以把指向源字符串的第2个指针声明为指针、数组名或字符串常量；而指向源字符串副本的第1个指针应指向**一个数据对象（如，数组名而不是地址）**，且该对象有足够的空间储存源字符串的副本。
...
* `sprintf()`函数声明在`stdio.h`中。该函数和`printf()`类似，但是它是把数据写入字符串，而不是打印在显示器上。因此，该函数可以把多个元素组合成一个字符串
* `sprintf()`的第一个参数是目标字符串的地址，其余参数和`printf()`相同，即格式化字符串和代写入项的列表。
* 相当于`sprintf()`将`printf()`输入到显示器上的内容存储到一个字符串内



### char \*strchr(const char\*s, int c);

* 该函数是一个字符指针类型的函数.也就是说,该函数的返回值是一个字符类型的指针变量
* 该函数的功能是,如果`s`字符串中包含`c`字符,该函数返回指向`s`字符串首位置的指针(末尾的空字符也是字符串的一部分,所以在查找范围内);如果在字符串`s`中未找到`c`字符,该函数返回空指针.



### char \*strbrk(const char\*s1, const char \* s2);

* 如果`s1`字符中包含`s2`字符串中的任意位置, 该函数返回指向`s1`字符串首位置的指针;如果在`s1`字符串中未找到任何`s2`字符串中的字符,则返回空字符.
...
* 按字母表顺序排序字符串
* 主要是用`strcmp()`函数来确定两个字符串的顺序。一般的做法是读取字符串函数、排序字符串并打印出来
...
int main(void){
    ...
    ststr(ptstr, ct);          // 字符串排序函数。这样排序只会改变指针的指向，不会改变原数组
    put("\nHere's the sorted list:\n");
    for (k = 0; k < ct; k++) puts(ptstr[k]);  //排序后的指针
    return 0;
}
/*字符串-指针-排序函数*/
...
* 排序过程把指向数组的指针`ptrst`重新排列，并未改变`input`

### 选择排序算法

* 采用选择排序算法来排序指针。利用`for`循环依次把每个元素与首元素比较，如果待比较的元素在当前首元素前面，则交换两者。循环结束时，首元素包含的指针指向机器排序序列最靠前的字符串。然后外层`for`循环重复这一过程
...
* `ctype.h`中有一系列与字符`char`相关的函数.虽然这些函数不能处理整个字符串,但是可以处理字符串中的字符.
...
* 进行数值运算需要使用数值形式,显示数字则需要用字符串形式
* 使用`atoi()`函数可以把字母数字转换成整数(需要include `stdlib.h`)
...
int main(int argv, char*argv[]){
    int i, times;
    if(argc < 2 || (times = atoi(argv[1])) < 1) printf("Useage: %s positive-number\n", argv[0]);
    else for(i=0; i<times; i++) puts("Hello, good looking!");
    return 0;
}
```

* 与`atoi()`类似的函数还有`atol, atof`等等.
* ANSI C还提供了一套更智能的函数:`strtol()`把字符串转换成`long`类型的值,`strtoul()`把字符串转换成`unsigned long`类型的值,`strtod()`把字符串转换成`double`类型的值.这些函数的只能之处在于识别和报告字符串中的首字符是否是数字.而且,`strtol()`和`strtoul()`还可以制定数字的进制



## 关键概念

* 字符串,无论是由字符数组, 指针还是字符串常量标识, 都储存为包含字符编码的一系列字节, 并且以空字符串结尾.
* C 提供库函数处理字符串,查找字符串并分析它们.
* **牢记**: 应该使用`strcmp()`进行比较, 使用`strcpy()`或`strncpy()`进行字符串复制



## 本章小结

* C 字符串是一系列`char`类型的字符, 一空字符`('\0')`结尾. `strlen()`函数可以统计字符串的长度,但并不包空字符`\0`
* 字符串常量(如`"hello, world"`)也叫作字符串字面量.可用于初始化字符数组.为了容纳末尾的空字符,**数组大小应该至少比容纳的数组长度多1**

* 函数使用指向字符串首字符的指针来表示待处理的字符串,通常传入的实际参数是数组名, 指针变量或双引号括起来的字符串.**无论哪种情况,传递的都是首字符的地址**
* C库中的字符串处理函数声明在`string.h`文件中;字符处理函数声明则在`ctype.h`文件中
* 给`main()`函数提供两个合适的形式参数`int main(int argc, char*argv[])`可以让程序访问命令行参数. 第一个参数是`int`类型的`argc`,其值是命令行的单词数量; 第二个参数是指针数组, 该数组的元素均为`char`类型的指针,指向命令行参数字符串.
...
* 程序与文件交互的形式是文件重定向
* 也可以在程序中打开文件, 然后使用特殊的I\O函数读取文件中的信息或把信息写入文件.
